...
abline(h = summary(pheno_covar_perm)[, "z68_shoots_mean_hn"], lty = 2)
```
...
temp <- pull_genoprobpos(geno_prob, "c5.loc137.5") %>% 
...
```{r}
p +
  geom_abline(intercept = coef(covar_qtl_m1)["aa"], slope = coef(covar_qtl_m1)["flowering_time_mean_hn"]) +
  geom_abline(intercept = coef(covar_qtl_m1)["bb"], slope = coef(covar_qtl_m1)["flowering_time_mean_hn"])
```
...
In conclusion: this might be an interesting (if minor) QTL that explains branchiness independent of flowering time. 
...
I would be tempted to fit these as the "final" QTL models, although I don't expect things will change that much. 
... to do ...