...
model_data <- training %>% 
  ...
  select(batch, nitrate_level, ril_id, trait_raw = flowering_senescence_interval) %>% 
...
Some outliers there, in particular one with over 5 SD away from the mean. We remove it.
...
We get warning of _boundary fit_, which we can see is because there's basically no variance on LN.
And even on HN the variance is probably small-ish.
...
Generally positive, with inter-quartile range between ~1.25 and 1.75 standard deviations. 
Quite fat lower tail.
...
The saturated model has lowest AIC. Several models including batch come on top.
...
- Batch seems to have an effect on this trait
- There's little evidence for variance on LN, so effectively one should just model HN data
...
test <- test %>% 
  mutate(predicted = predict(model_fits$n_ril_int_batch_intn, newdata = test, allow.new.levels = TRUE),
         observed = (flowering_senescence_interval - mean(model_data$trait_raw)) / sd(model_data$trait_raw)) %>% 
  drop_na(observed)
