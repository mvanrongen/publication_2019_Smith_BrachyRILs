...
model_data <- training %>% 
  ...
  select(batch, nitrate_level, ril_id, trait_raw = senescence_height) %>% 
...
there's an outlier that is 10 standard deviations away from everything else! 

```{r}
# check outlier
training %>% 
  select(identifier, batch, nitrate_level, ril_id, trait = senescence_height) %>% 
  drop_na() %>% 
  arrange(-trait) %>% 
  head()
```
...
This is removed before fitting model:

```{r}
# remove outlier
model_data <- model_data %>% filter(trait < 5)
...
model_pred %>% 
  ...
  scale_colour_manual(values = c("black", "steelblue", "firebrick"))
```
...
Generally negative, with inter-quartile range between -1 and -0.5 standard deviations. 
...
Most of the weight is in the very complex model, with batch seemingly being important.
...
Again, there's quite a high correlation between slopes and intercepts.


## Conclusion

- Batch seems to have an effect on this trait - this is good to be aware of because it means there's added unpredictability in new data (perhaps micro-environmental variation?)
- There's little GxE variation (although some)
...
test <- test %>% 
  mutate(predicted = predict(model_fits$n_ril_int_batch_intn, newdata = test, allow.new.levels = TRUE),
         observed = (senescence_height - mean(model_data$trait_raw)) / sd(model_data$trait_raw)) %>% 
  drop_na(observed)
