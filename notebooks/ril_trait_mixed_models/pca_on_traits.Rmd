...
model_data_pca <- model_data %>% 
  select(-batch, -nitrate_level, -ril_id) %>% 
  prcomp(scale = TRUE)

library(ggfortify)
autoplot(model_data_pca, data = model_data,
         colour = "nitrate_level", loadings = TRUE, loadings.label = TRUE)
```

Check what the eigenvectors (or "eigen traits") are for those PCs:
...
```{r}
model_data <- model_data_pca %>% 
  augment(model_data) %>% 
  # select variables of interest - rename trait of interest
  select(batch, nitrate_level, ril_id, trait = .fittedPC1)
...
Yes, substantial and consistent batch effect, with batches 3-4 lower on both nitrate treatments.
Also, batch 5 higher on LN. 
This is reflecting issues seen in individual trait analysis...
