...
model_data <- training %>% 
  ...
  select(batch, nitrate_level, ril_id, trait_raw = seed_weight_g) %>% 
...
No clear outliers. But clearly there's variance heterogeneity... Oh no, this is harder to model :(
...
Generally positive, with inter-quartile range between ~1.4 and 1.75 standard deviations. 
...
Batch 6 HN has much higher seed weight! What happened there?
...
- Batch seems to have an effect on this trait, but mostly might be because of batch 5 HN
...
test <- test %>% 
  mutate(predicted = predict(model_fits$n_ril_int_batch_intn, newdata = test, allow.new.levels = TRUE),
         observed = (seed_weight_g - mean(model_data$trait_raw)) / sd(model_data$trait_raw)) %>% 
  drop_na(observed)
